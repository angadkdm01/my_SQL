...
INSERT INTO `people` VALUES ('Barr Faughny','SP01','Yummies','Hyderabad'),('Dennison Crosswaite','SP02','Yummies','Hyderabad'),('Gunar Cockshoot','SP03','Yummies','Hyderabad'),
                            ...
                            ('Kaine Padly','SP07','Delish','Hyderabad'),('Ches Bonnell','SP08','','Hyderabad'),('Andria Kimpton','SP09','Jucies','Hyderabad'),
                            ...
                            ('Marney O\'Breen','SP16','Jucies','Wellington'),('Rafaelita Blaksland','SP17','','Wellington'),('Madelene Upcott','SP18','Jucies','Wellington'),
                            ('Beverie Moffet','SP19','Jucies','Seattle'),('Oby Sorrel','SP20','Jucies','Seattle'),('Mallorie Waber','SP21','','Seattle'),
                            ('Jehu Rudeforth','SP22','','Seattle'),('Van Tuxwell','SP23','Yummies','Seattle'),('Roddy Speechley','SP24','Delish','Seattle'),
                            ('Camilla Castle','SP25','Delish','Seattle'),('Janene Hairsine','SP26','Delish','Paris'),('Niall Selesnick','SP27','Jucies','Paris'),
                            ('Ebonee Roxburgh','SP28','','Paris'),('Zach Polon','SP29','Yummies','Paris'),('Orton Livick','SP30','Yummies','Paris'),
...
drop trigger prevent_duplicate_spid;


#######################################
-- --Questions----
#Q1.Renaming a column name of table `geo`
alter table geo rename column Geo to Country; 

#addign constraints to various columns
alter table geo modify column Country text unique;
...
#Q2.droppping a column from `geo`
alter table geo drop column Region;

#Q3.adding a new column
alter table geo add column Region text;
alter table geo add column last_updated datetime;

#Q4.creating a trigger to show last_updated time
delimiter $$
create trigger geo_update
...
#Q5.updating column GeoID using in operator
update geo set Region = "APAC" where GeoID in('G1', 'G4', 'G5'); 
update geo set Region = "Americas" where GeoID = 'G2' or GeoID = 'G3'; 
update geo set Region = "Europe" where GeoID = 'G6'; 



#Q6.changing position of column GeoID at first
alter table geo modify column Region text after GeoID;

####Aggregate Functions
#.How many salesersons are there in total in the people table?
select count(*) from people;

#Q7.What is the number of salespersons in each team in the people table?
select Team, count(SPID) from people group by Team;


#.How many distinct Ctegories are there in the products table?
select count(distinct Category) from products;

####Operators
#.retrieving all records where GeoID = 3
select * from sales where GeoID = 'G3';

#.Find all salespersons in the people table who are not part of the 'Yummies' team.
select * from people where Team != 'Yummies';

#Q8.retrieving all products which has customers greater than equal to 20
select Product from products where PID in (select PID from sales where Customers >= 20);

#Q9.retrieving all salesperson having location includes letter 'H' at first index of word 
select * from people where location like "H%";

#### limit Clause
#.Write a query to retrieve the first 2 rows from the people table.
select * from people limit 2;

#10.Fetch rows 6 to 10 from the products table.
select * from products limit 5 offset 5;

#### order by caluse
#.Write a query to retrieve all rows from the sales table sorted in descending order by the Amount column.
select * from sales order by Amount desc;

#.Retrieve all rows from the people table sorted first by the Team column in ascending order and then by the Salesperson column in descending order.
select * from people order by Team asc, Salesperson  desc;

#Q11.Retrieve the 3 locations with the salesperson in the people table, sorting by the count of salespeople in ascending order.
select Location, count(SPID) as num_salesperson from people group by Location order by num_salesperson asc limit 3;

#### distinct clause
#Retrieve all unique combinations of Team and Location from the people table.
select distinct Team, Location from people;

#Q12.Find all unique Regions from the geo table where the GeoID starts with 'G'.
select distinct Category from products where Cost_per_box > 2;

#.list all unique Locations from the people table without any duplicates
select distinct Location from people;

#### group by clause
#.Retrieve the number of salespersons in each Team within each Location.
select Team, Location, count(SPID) from people group by Team, Location;

#Q13.Calculate the total count of salespersons in each Team from the people table.
select Team, count(SPID) as total_salepersons from people group by Team;

#.Write a query that counts the number of regions in the geo table and groups them by whether they are more than one word (i.e., containing a space). Use a CASE statement to define these groups.sql
select 
	case
		when Region like "% %" then "Multi-word Region"
        else "Single-word Region"
	end as Region_Type, count(GeoID) as num_regions from geo group by Region_Type;
    
#.Calculate the average team size (number of members) for each Location in the people table
select Location, avg(team_size) as avg_team_size from (select Location, count(SPID) as team_size from people group by Location, Team) as team_sizes group by Location;
	
#### group by -having
#.Find all teams with more than 4 salespersons.
select Team, count(SPID) as team_size from people group by team having count(SPID) > 4;

#Q14.Identify locations where the count of salespersons across all teams is greater than 8.
select Location, count(SPID) from people group by Location having count(SPID) > 8;

#### sub-query
#Q15.retrieve all products having amount greater than 4000.
select * from products where PID IN (select PID from sales where Amount > 4000); 
#Retrieve all rows from the people table where the salesperson is not part of any team (i.e., their Team is an empty string or NULL).
select * from people where Team in (select Team from people where Team = '');


#### joins
#.Display the details of Product has 2nd highest Amount.
select pr.Product, s.Amount
from
products p join sales s
on pr.PID = s.PID where s.Amount = (select max(Amount) from sales where Amount < (select max(Amount) from sales));

#retrieve records of Bars shipment only
select *
from
products pr join sales s
on pr.PID = s.PID
where pr.Category = "Bars";

#retrieve all records of Salesperson = Barr Faughny
#Bar Barr(Person) Shipments
select p.Salesperson, p.Team, p.SPID, s.SPID, s.Amount, s.Customers, pr.Category, pr.Product, pr.PID from sales s
join people p on s.SPID = p.SPID and p.Salesperson = "Barr Faughny" 
join products pr on s.PID = pr.PID and pr.category = "BARS"; 

#Q16.Bar Barr(Person) Shipments
select p.Salesperson, p.Team, p.SPID, s.SPID, s.Amount, s.Customers, pr.Category, pr.Product, pr.PID from sales s
join people p on s.SPID = p.SPID  
join products pr on s.PID = pr.PID 
where pr.category = "BARS" and p.Salesperson = "Barr Faughny" order by s.Amount desc;

##### stored procedures
#Create a MySQL stored procedure to list all products from the sales table and their total sales amounts across all regions.
delimiter $$
create procedure get_totalSales_byProduct()
begin
	select s.PID, sum(s.Amount) as Total_Sales from sales s group by s.PID;	
end$$
delimiter ;
call get_totalSales_byProduct();

#Q17.Create a MySQL stored procedure to insert a new salesperson into the people table.
delimiter $$
create procedure insert_new_salesper(
 in p_Salesperson text,
 in p_SPID varchar(6),
 ...
 in p_Location text)
 begin
	insert into people(Salesperson, SPID, Team, Location) values(p_Salesperson, p_SPID, p_Team, p_Location);
end$$
delimiter ;
call insert_new_salesper('Angad', "SP34", "Mango", "Ratnagiri");

#Q18.show detail of specific category
delimiter $$
...
#### stored functions
#Q19. Create a stored function in MySQL that calculates the average sales amount of all salespeople in a specific region. 
   #The function should take the region as a parameter and return the average amount
delimiter $$
create function avg_sales_by_region(
e_Region text)
returns decimal(10,2)
...
#### trigger
#Q20.creating a table name Geo2 for using cursor
create  table geo2(GeoID text, Region text);
delimiter $$
create trigger copy_geo_to_geo2
after insert on geo 
for each row
begin
	insert into geo2 (GeoID, Region) values(new.GeoID, new.Region);
end$$
delimiter ;
insert into geo(GeoID, Region) values("G07", "Harchiri");
select * from geo2;

####Views
#Q21.created a view to hide secret data and show necesaary data only 
create view show_product_info as 
select g.Region, g.Country, p.Salesperson, p.Team, p.Location, pr.Product, pr.Category, pr.Size, s.SaleDate, s.Amount, s.Customers, s.Boxes
from
geo g join sales s on s.GeoID = g.GeoID join
people p on p.SPID = s.SPID join
products pr on pr.PID = s.PID;
